...
export function Activities({ tripDetails }: Props) {
    ...
    try {
      ...
      const activitiesToSectionList = activities.map((dayActivity) => ({
        ...
        data: dayActivity.activities.map((activity) => ({
          ...
          date: dayjs(activity.occurs_at).format("DD/MM/YYYY"),
  ...
  return (
    <View className="flex-1">
      <View className="w-full flex-row mt-5 items-center">
        <Text className="text-zinc-50 text-2xl font-semibold flex-1">
          Atividades
        </Text>

        <Button onPress={() => setShowModal(ModalState.NEW_ACTIVITY)}>
      ...
      <Modal
        ...
        subtitle="Crie uma nova atividade para a sua viagem"
        visible={showModal === ModalState.NEW_ACTIVITY}
